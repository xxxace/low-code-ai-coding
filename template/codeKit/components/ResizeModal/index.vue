<template>
  <vxe-modal :className="`resize-modal vxe-table--ignore-clear ${props.noOverflow ? 'no-overflow-modal' : ''}`"
             ref="modalRef" size="mini" show-zoom :show-minimize="false" :lock-view="false" :mask="isShowMask"
             :destroy-on-close="true" v-bind="modelOptions" resize @zoom="handleZoom"
  >
...
.vxe-modal--wrapper.is--padding.no-overflow-modal .vxe-modal--content,
.vxe-modal--wrapper.no-overflow-modal .vxe-modal--content {
  padding: 0;
}

.vxe-modal--wrapper.resize-modal.is--padding .vxe-modal--body-default {
  padding: 0;
}
</style>
