  ...
  <div class="board">
    <div class="headerVue">
      <div class="p-10 header">
        <div class="title text-center"> 织机发料看板 </div>
      ...
      <div class="flex-between box">
        ...
        <div class="flex">
          <div class="flex-align-center mr20">
            <h3>总需料数:</h3>
            <div class="box-r ml10 text-bold c-green" style="font-size: 20px">{{ QTYSUM }} 磅</div>
          </div>
          <!-- <div class="flex-align-center ml10">
            <h3>低效率:</h3>
    ...
    <div class="flex">
      ...
      <rmaterVue class="ml10" style="min-width: 300px" :res-data="dataList" title="发料率" />
    </div>
    <div class="flex mb10">
      <div
        class="box-r box-s black-box p-10 overflow flex-column mt10"
        style="height: 350px; width: 55%"
      >
        <div class="text-bold text-center text-llg c-white">物料信息</div>
        <div class="flex1 overflow mt10">
          <el-table
            ...
            height="100%"
            class="black-table"
            :row-class-name="tableItemRowClassName"
            show-summary
            :summary-method="getSummaries"
          >
            <el-table-column prop="CLNAME" label="颜色" width="120" />
            <el-table-column prop="MATNO" label="毛料编码" width="120" />
            <el-table-column prop="MATNAME" show-overflow-tooltip label="毛料名称" />
            <el-table-column prop="SPECNO" label="色号" width="120" />
            <el-table-column prop="MSNAME" w label="色名" width="80" />
            <el-table-column prop="VAT" label="缸号" width="120" />
            <el-table-column prop="BODYWT" label="需料数(磅)" width="95" />
            <el-table-column prop="WELTWT" label="发料数(磅)" align="right" width="95" />
            <el-table-column prop="QFLQTY" label="欠发料数(磅)" align="right" width="105" />
          </el-table>
        </div>
      </div>
      <div class="box-r box-s black-box p-10 overflow flex-column mt10 ml10 flex1">
      ...
      <!-- <smaterVue class="flex1 ml10 mt10" :res-data="smaterList" title="收料率" /> -->
    </div>
  </div>
</template>
<script setup>
...
const queryData = async () => {
  ...
  const { DBQUERY, SORTBYCONTENT } = remoteSqlMap['织机物料信息']
  const { data, statusCode } = await searchData({
    sql: DBQUERY,
    params: WHERE.join(' AND '),
    sortby: SORTBYCONTENT
  })
  if (statusCode == 1) {
    dataList.value = data
...
const CellClassName = ({ row, rowIndex, column }) => {
  if (row.YXJ == '102 急单' && column.property == 'YXJ') {
    return 'cellRed'
  }
  return ''
}
const tableCellClassName = () => {}
const rmaterList = ref()
const smaterList = ref([])
...
const getSummaries = ({ columns, data }) => {
  ...
  columns.forEach((column, index) => {
    ...
    if (['VAT'].includes(column.property)) {
      return (sums[index] = '')
    }
...
const tableItemRowClassName = ({ row, rowIndex }) => {
  if (rowIndex == 0) {
    return ''
  }
  return ''
