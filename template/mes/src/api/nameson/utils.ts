export * from '@/hooks/nameson/useFetchSqlV2'
import { ElMessage, ElMessageBox } from 'element-plus'
import type { SearchDataHandler } from '@/hooks/nameson/useFetchSqlV2'
...
export async function fetchDataList<T>(
  ...
  const errorHandler = function (error: any, type: 'warning' | 'error') {
    if (!option.slient) {
      ...
      } else if (message.indexOf('数据为空') === -1) {
...
export async function fetchDataItem<T>(
  ...
  try {
    const res = await promiseResponse
    if (res.statusCode == '1') {
      return ((res.data as any)[0] || option.defaultValue || {}) as T
    } else {
      return errorHandler(res.message, 'warning')
    }
  } catch (e) {
